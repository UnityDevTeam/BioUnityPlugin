...
{
    [MenuItem("My Commands/Add molecule %i")]
    static void FirstCommand()
    {
        var pdbName = new[] {"2RH1", "1OKC", "2K4T", "2FP4", "2OAU", "p3"};
        GameObject.Find("Main Script").GetComponent<SimpleScript>().AddMolecule(pdbName[Random.Range(0, 5)]);
    }
}
...
public class SimpleScript : MainScript
{
    public void AddMolecule(string pdbName)
    {
        ...
        this.AddMoleculeInstance(pdbName, new Vector3(Random.Range(-25, 25), Random.Range(-25, 25) + 25, Random.Range(-25, 25)), Random.rotation);
    }
